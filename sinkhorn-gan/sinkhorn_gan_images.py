...
def run_gan(args,loss, batch_size, epsilon = 1, niter_sink = 1):
    ...
    ######################################################################
    # Arguments
    log_dir = '{0}_{1}_eps{2}_niter{3}_batch{4}'.format(args.experiment,loss,epsilon,args.max_iter,batch_size)
    ...
    for t in range(args.max_iter):
        ...
        while (i < len(trn_loader)):
            ...
            if gen_iterations % 500 == 1:
                ...
                imgfilename = '{0}_{1}_eps{2}_niter{3}_batch{5}/progress/imglist_{4}'.format(args.experiment,loss,epsilon,args.max_iter,gen_iterations , batch_size)
...
batch_size_list = [3200, 1000, 500, 2000, 4000]
niter_list = [1,10,100]
